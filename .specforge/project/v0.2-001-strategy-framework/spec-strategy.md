...
| 回测方法     |      完整资金循环模拟       |      ❌ 不可回测 (BacktestRunner 拒绝 RiskStrategy)      | 无账户 → 无回测语义 |
...
回测启动时设置的策略参数在转仿真/实盘/dry-run 时**自动带入**，不需要、也不允许单独设置。

> **AC**: 无（行为由 [test-plan.md §6.4.5](./test-plan.md) L1 paper E2E 覆盖）
...
> **AC**: 无（行为由 [test-plan.md §6.4.5](./test-plan.md) L1 paper E2E 覆盖；具体 AC 在域 1 Commit B 集中补 FR-030/FR-040）
...
> **AC**: 无（行为由 [test-plan.md §1.5 三种下单方式表 + §6.4 L1 paper E2E](./test-plan.md) 覆盖）
...
> **AC**: 无（声明性 FR，由 [test-plan.md §1.5 + §6.3 跨模式差异测试](./test-plan.md) 覆盖）
...
> **AC**: 无（行为由 [test-plan.md §1.3 L1 paper E2E + §3.3 回测 ground truth](./test-plan.md) 覆盖）
...
> **AC**: 无（行为由 [test-plan.md §3.5 风控 ground truth + §6.4 L1 paper E2E](./test-plan.md) 覆盖；账户/不可回测/不可独立启动 已在 AC-013 段）
...
持仓的"成本基准"用于风控策略（FR-110 成本止损）判断是否触发卖出。**算法**（用户 2026-06-21 拍板：经典方案 A — 买入时加权更新，部分卖出后剩余持仓成本不变）：

#### 算法公式

**F-CB-1 买入时加权更新**：
```
∀ asset, 在已有持仓上买入 (old_qty > 0):
    new_qty   = old_qty + buy_qty
    new_price = (old_qty × old_cost_basis + buy_qty × buy_price) / new_qty
    cost_basis ← new_price
```

**F-CB-2 部分卖出时成本不变**：
```
∀ asset, 部分卖出 (0 < sell_qty < old_qty):
    new_qty   = old_qty - sell_qty
    cost_basis ← old_cost_basis   # 关键: 剩余持仓成本**不变**
```

**F-CB-3 全部卖出时清仓**：
```
∀ asset, 全部卖出 (sell_qty == old_qty):
    删除该持仓记录 (cost_basis 不再保留)
```

**F-CB-4 首次建仓**（old_qty == 0 时买入）：
```
cost_basis ← buy_price   # 按本次买入价初始化
```

#### 边界条件

- **同日反复买卖**（同一 asset）: 所有买入按 F-CB-1 累加更新；卖出按 F-CB-2/F-CB-3 处理
- **多次部分卖出后再次买入**: 先 F-CB-2 保留成本，再 F-CB-1 加权更新
- **停牌/退市/退市后重新上市**: 现有实现会清空该 asset 的持仓（具体行为见 FR-170，本 FR 不重复定义）
- **融资融券**: v0.2 不涉及（成本基准不包含融资费用）

#### 持久化

- 存储位置: [interfaces.md §4.4 positions 表](./interfaces.md) 的 `price` 字段
- 写入时机: 每次成交后由撮合层立即更新
- **不**等于"已实现利润/亏损回算后的移动平均"——后者是 P&L 概念，本 FR 不涉及

#### 与 FR-110 成本止损的引用

FR-110 成本止损的"跌破买入价 k%"中的"买入价"= 本 FR 的 `cost_basis` 字段（由 F-CB-1/2/3/4 维护）。即:

- 触发条件: `last_price <= cost_basis × (1 − k%)`
- **不**基于"加权历史买入价" 或 "已实现利润回算成本" — 那些是其他实现方案，本 FR 不采用

> **AC 覆盖**: 行为由 [test-plan.md §6.4.1 / §6.4.5](./test-plan.md) L1 paper E2E 覆盖，AC 在域 2 (FR-090/100/110) 集中补齐时引用本 FR.
>
> **为什么选 A 经典方案**: 实现最简单，不容易出错，符合国内基金/股票交易常用语义；部分卖出后剩余持仓成本不变是绝大多数零售交易者的预期，也是会计上"成本基础法"的标准做法。
...
> **AC**: 无（行为由 [test-plan.md §1.3 L1 paper E2E + 配置契约](./test-plan.md) 覆盖；运行时参数存储见 [interfaces.md §4.5 assets.principal](./interfaces.md)）
...
> **AC**: 无（行为由 [test-plan.md §1.3 L1 paper E2E + 调度契约](./test-plan.md) 覆盖；30 个回测结果上限由 [interfaces.md §4.1 回测结果 JSON](./interfaces.md) 文件系统管理）
...
> **AC**: 无（行为由 [test-plan.md §1.3 L1 paper E2E + 调度契约](./test-plan.md) 覆盖）
...
> **AC**: 无（行为由 [test-plan.md §1.3 L1 paper E2E + §3.5 风控 ground truth](./test-plan.md) 覆盖；AC-013-XX 已涵盖风控生命周期）
