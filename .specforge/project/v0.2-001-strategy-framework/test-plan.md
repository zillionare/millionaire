...
4. **paper/live E2E 的 marker 管控**（详见 §5.5）
   - paper/live 相关 E2E 测试**必须**标注 §5.5.1 定义的三类 marker（`e2e_paper` / `e2e_gateway` / `e2e_live_smoke`）之一，无 marker 的 paper/live 测试阻塞 merge。
...
3. §5 定义的 paper/live E2E 三层结构中:L1(`e2e_paper`)与 L2(`e2e_gateway`)在默认 CI 中可重复通过;L3(`e2e_live_smoke`)在 nightly/手动环境可运行,默认 deselect 不得以无 issue 的 `skip` 实现(见 §5.5.3)。
...
1. **不得**在测试侧用 `mock.patch` 内部符号强行打通(违反 §5.2.1 边界铁律)。
...
├── virtual_clock.py        # §5.4.1 替换真实墙钟
├── replay_market_data.py   # §5.4.2 替换 gateway 实时行情
├── order_observer.py       # §5.4.3 只读订单观察者(可选)
├── fake_gateway.py         # §5.4.4 替换 gateway 真柜台(L2)
└── runtime_factory.py      # §5.4.5 编排器(组装 + 时间推进)
```
