# Spec — 交易分册

- **Spec ID**: v0.2-001-strategy-framework
- **分册**: trading (交易规则、数据源、评估、账户、UI、通知、安装)
- **FR 范围**: FR-140 ~ FR-180, FR-190, FR-210 ~ FR-220, FR-270 ~ FR-485
- **根 spec**: [spec.md](./spec.md)

## 功能需求

<a id="fr-140"></a>
### FR-140 交易规则 — 价格（涨跌停）

| 有效需求 | 可测性 | 是否已决定 |
| -------- | ------ | ---------- |
| ✅        | ✅      | ✅          |

- 以数据 `up_limit` / `down_limit` 字段为准（普通 ±10%、ST ±5%、创/科 ±20% 由数据源提供，框架不内置规则）
- 限价单指定价必须落在 `[down_limit, up_limit]` 内，否则下单直接拒绝
- 开盘即涨停的标的买入单不撮合；开盘即跌停的标的卖出单不撮合
- `is_st=true` 股票不做下单限制（策略自行过滤）


---

<a id="fr-150"></a>
### FR-150 交易规则 — 数量

| 有效需求 | 可测性 | 是否已决定 |
| -------- | ------ | ---------- |
| ✅        | ✅      | ✅          |

- 买入数量必须是 100 整倍数；下单前自动向下取整（不足 100 部分丢弃）；取整后为 0 则下单失败
- 卖出一般须 100 整倍数；**清仓（卖出全部可用持仓）与零股（不足一手的零散股）允许例外**
- 仿真/实盘实际成交数量受涨跌停/停牌/成交量约束，可能少于委托
- 回测中订单要么全成交，要么作废（不考虑成交量）


---

<a id="fr-160"></a>
### FR-160 交易规则 — 时间

| 有效需求 | 可测性 | 是否已决定 |
| -------- | ------ | ---------- |
| ✅        | ✅      | ✅          |

- **T+1**：当日买入的持仓当日不可卖；次日起方可卖。框架按"可卖持仓"与"在途持仓"分别记账
- 非交易时段（含午休、周末、节假日）生成的信号，订单顺延至下一交易时点（具体行为由 FR-050/060/070 决定）


---

<a id="fr-170"></a>
### FR-170 交易规则 — 特殊状态（停牌）

| 有效需求 | 可测性 | 是否已决定 |
| -------- | ------ | ---------- |
| ✅        | ✅      | ✅          |

停牌股票（数据缺失或 `volume=0`）不可下单；持仓中的停牌股票按停牌前最后收盘价估值。


---

<a id="fr-180"></a>
### FR-180 交易规则 — 资金

| 有效需求 | 可测性 | 是否已决定 |
| -------- | ------ | ---------- |
| ✅        | ✅      | ✅          |

- 卖出回款当日可用于继续买入（A 股现金账户规则）
- 印花税（卖方收取，按比率）、佣金（按比率，不低于单笔最低佣金）按 FR-200 配置扣除，扣减发生在成交后立即结算
- 资金校验在虚拟账户层进行：买入下单时检查虚拟账户可用资金是否足以覆盖（成交金额 + 佣金）


---

<a id="fr-190"></a>
### FR-190 交易规则 — 回测 vs 实盘的差异

| 有效需求 | 可测性 | 是否已决定 |
| -------- | ------ | ---------- |
| ✅        | ✅      | ✅          |

回测时上述规则由框架仿真全部实施；仿真/实盘时，时间/价格/数量规则由交易所/柜台强制保证，框架只在下单前做预校验以避免明显错误委托被发出。撮合差异见 FR-080。


---

<a id="fr-210"></a>
### FR-210 虚拟账本（按 qtoid 归因）

| 有效需求 | 可测性 | 是否已决定 |
| -------- | ------ | ---------- |
| ✅        | ✅      | ✅          |

每个独立策略在 live/paper 中运行时拥有一个虚拟账户，配置独立本金作为资金上限。所有虚拟账户共享真实账户的总资金（用户自行保证各策略本金之和 ≤ 真实账户可用资金；Millionaire 不做跨虚拟账户资金分配校验）。Millionaire 为每个独立策略维护按 `qtoid` 归因的虚拟账本，用于策略级资产/持仓/委托/成交/收益评估/dry-run/并行仿真。除非特别说明，系统中的"策略账户"均指这套虚拟账本，而非柜台原始总账户。


---

<a id="fr-220"></a>
### FR-220 手工交易/补单/风控卖出的归属

| 有效需求 | 可测性 | 是否已决定 |
| -------- | ------ | ---------- |
| ✅        | ✅      | ✅          |

手工交易、补单和风控卖出都必须归属到某个独立策略账户，并写入该策略的虚拟账本。柜台原始账户信息主要用于总览和排障，不直接替代策略级归因结果。


---
<a id="fr-270"></a>
### FR-270 数据源 — 行情（tushare）

| 有效需求 | 可测性 | 是否已决定 |
| -------- | ------ | ---------- |
| ✅        | ✅      | ✅          |

日线（个股、指数）核心历史数据。字段：OHLCV + amount + adjust + is_st + up_limit + down_limit。Parquet 存储，按年分区。数据源：**tushare**。tick/分钟/30 分钟仅 live/paper 模式下使用，不保存。

---

<a id="fr-280"></a>
### FR-280 数据源 — 参考数据（tushare）

| 有效需求 | 可测性 | 是否已决定 |
| -------- | ------ | ---------- |
| ✅        | ✅      | ✅          |

- 交易日历（所有交易所）
- 证券列表（全市场代码/名称/拼音/上市退市日期）
- is_st 标记（ST / *ST）

数据源：**tushare**。

---

<a id="fr-290"></a>
### FR-290 数据源 — 复权与涨跌停（tushare）

| 有效需求 | 可测性 | 是否已决定 |
| -------- | ------ | ---------- |
| ✅        | ✅      | ✅          |

- adjust（复权因子）用于前/后复权计算
- 涨跌停价：A 股特有，用于撮合判断与限价单比较

数据源：**tushare**。

---

<a id="fr-300"></a>
### FR-300 数据源 — 实时行情（qmt-gateway）

| 有效需求 | 可测性 | 是否已决定 |
| -------- | ------ | ---------- |
| ✅        | ✅      | ✅          |

通过外部 `qmt-gateway` 项目获取实时 tick 与分钟线。**仅 live/paper 模式使用**。

---

<a id="fr-310"></a>
### FR-310 数据同步任务

| 有效需求 | 可测性 | 是否已决定 |
| -------- | ------ | ---------- |
| ✅        | ✅      | ✅          |

- 定时执行日线行情、证券列表、交易日历、st、涨跌停历史数据同步
- 盘前获取当日涨跌停价格与复权因子
- 定时任务可配置;运行有报告跟踪;**错误展示已迁移到 [v0.2-002-ui/spec.md UI-FR-310-UI](../v0.2-002-ui/spec.md)**
- 任务错过执行(错误纠正后)可重新执行
- 支持数据补录

---

<a id="fr-320"></a>
### FR-320 数据完整性校验及报告

| 有效需求 | 可测性 | 是否已决定 |
| -------- | ------ | ---------- |
| ✅        | ✅      | ✅          |

后台任务定时执行数据完整性校验(缺日、重复日、字段空值率等);**报告展示已迁移到 [v0.2-002-ui/spec.md UI-FR-320-UI](../v0.2-002-ui/spec.md)**。

---

<a id="fr-330"></a>
### FR-330 数据查询支持

| 有效需求 | 可测性 | 是否已决定 |
| -------- | ------ | ---------- |
| ✅        | ✅      | ✅          |

- 支持用户查询交易日历
- 支持用户按名字/数字/拼音模糊查询个股基本信息
- 支持用户查询个股历史行情;**K 线图绘制已迁移到 [v0.2-002-ui/spec.md UI-FR-330-UI](../v0.2-002-ui/spec.md)**

---

<a id="fr-340"></a>
### FR-340 评估指标 — 回测（独立策略）

| 有效需求 | 可测性 | 是否已决定 |
| -------- | ------ | ---------- |
| ✅        | ✅      | ✅          |

独立策略在可回测的数据粒度（日线）下支持回测。指标：
- 年化收益率
- 最大回撤
- Sharpe 比率
- Sortino 比率
- Calma 比率
- 胜率
- 盈亏比
- 交易次数
- 基准对比（如沪深 300，用于判断策略 alpha）

> 风控策略的回测评估见 FR-360（超额收益，Triple Barrier）。

---

<a id="fr-350"></a>
### FR-350 评估指标 — 实盘/仿真

| 有效需求 | 可测性 | 是否已决定 |
| -------- | ------ | ---------- |
| ✅        | ✅      | ✅          |

独立策略（含日线、日内两类用户场景）在实盘/仿真运行时，**使用与 FR-340 相同的指标**进行评估。数据来源为实盘/仿真实际成交记录。

---

<a id="fr-360"></a>
### FR-360 评估指标 — 风控策略

| 有效需求 | 可测性 | 是否已决定 |
| -------- | ------ | ---------- |
| ✅        | ✅      | ✅          |

风控策略在 paper/live 下产生的评估指标。**仅在 paper/live 下计算**（v0.2 不支持 RiskStrategy 回测，评估自然仅 paper/live）。

#### 公式定义（其他文档通过 [F-TB-N] 引用）

**F-TB-1** up 屏障触发后的超额收益
- 触发条件: `barrier_hit = "up"`(价格先触达上界)
- 公式: `excess_return = −1 * (up_threshold / 100)`

**F-TB-2** down 屏障触发后的超额收益
- 触发条件: `barrier_hit = "down"`(价格先触达下界)
- 公式: `excess_return = +1 * (down_threshold / 100)`

**F-TB-3** 观察期到但未触发任何屏障
- 触发条件: `barrier_hit = "expire"`(N 日内未触达任何屏障,按 T_n 日收盘价退出)
- 公式: `excess_return = P_sell / Close_n − 1`
- 语义: 收盘价比卖出价低 → 风控取得**正**收益(避开了下跌);收盘价比卖出价高 → **负**收益(错过了上涨)

**F-TB-4** up 屏障触发判断(日线 / 回测)
- `high ≥ P_sell * (1 + up_threshold / 100)` → 屏障触发
- 一日内同时触达上界和下界: 以开盘价更接近者为准(见 FR-013 / story §1.9)

**F-TB-5** down 屏障触发判断(日线 / 回测)
- `low ≤ P_sell * (1 − down_threshold / 100)` → 屏障触发

**变量定义**:
- `P_sell`: 卖出价
- `up_threshold` / `down_threshold`: 上下阈值,**百分点**表示(如 5.0 表示 5%)
- `Close_n`: T_n 日收盘价(N=0 即当日收盘)
- `high` / `low`: 当日最高 / 最低价

**单位说明**: `up_threshold` / `down_threshold` 在数据库 / 策略参数中以**百分点**存储(与 `default_config()` 一致); `excess_return` 是**小数**比率(如 0.05 = 5%)。

> **引用约定**: 后续 interfaces.md / acceptance.md / test-plan.md 通过 `[F-TB-N]` 引用公式,不再复制公式主体。

#### 评估维度

| 维度 | 定义 | 来源 |
| --- | --- | --- |
| **超额收益** | Triple Barrier 公式（详见 FR-013 / story §1.9） | FR-013 |
| **触发次数** | 风控策略触发卖出的总次数 | FR-125 |
| **触发原因分布** | 按 `reason` 字段分组的触发次数（如"跌停"/"破位"等用户自定义标签） | FR-125 |
| **按开启区间切分** | 每个 stop → start 周期记为一个 `activation_id`;**同一 `activation_id` 内的所有触发事件独立累计**,不与前一个区间合并 | FR-013 / FR-125 |

#### 持久化字段

风控评估表（数据库或 Parquet）至少包含以下字段：

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `activation_id` | `str` (UUID 或自增 id) | 唯一标识一个"开启区间" |
| `risk_strategy_id` | `str` | 风控策略的 `strategy_id` |
| `host_strategy_id` | `str` | 宿主独立策略的 `strategy_id` |
| `event_id` | `str` (PK) | 单次风控触发事件 id |
| `asset` | `str` | 标的代码 |
| `trigger_ts` | `datetime` | 触发时间戳 |
| `trigger_price` | `float` | 触发价 |
| `reason` | `str` | 触发原因（来自 `sell_host_position(reason=...)`） |
| `excess_return` | `float \| null` | Triple Barrier 算出的超额收益；N=0 时仿真/实盘下立即计算；N>0 时等 N 日后回填 |
| `finalized_at` | `datetime \| null` | 超额收益最终确认时间（n>0 时 N 日后回填；n=0 时 = `trigger_ts`） |

#### 关闭区间的处理

- 风控被"单独停止" → 当前 `activation_id` 立即关闭,不再接收新事件;已发订单**不撤回**
- 重新启动 → 分配**新** `activation_id`,新事件归新区间
- 同一 `activation_id` 内的所有 `excess_return` 求和,作为该区间的总贡献

> **关联 AC**: [acceptance.md FR-360 AC-360-01 ~ 03](./acceptance.md)

---

<a id="fr-370"></a>
### FR-370 可视化 — 核心图表

> **已迁移到 [v0.2-002-ui/spec.md UI-FR-370](../v0.2-002-ui/spec.md)**。

---

<a id="fr-380"></a>
### FR-380 可视化 — 回测进度

> **已迁移到 [v0.2-002-ui/spec.md UI-FR-380 / UI-FR-080](../v0.2-002-ui/spec.md)**。

---

<a id="fr-390"></a>
### FR-390 账户总览（多策略维度）

> **已迁移到 [v0.2-002-ui/spec.md UI-FR-390](../v0.2-002-ui/spec.md)**。

---

<a id="fr-400"></a>
### FR-400 委托与成交记录

> **已迁移到 [v0.2-002-ui/spec.md UI-FR-400](../v0.2-002-ui/spec.md)**。

---

<a id="fr-410"></a>
### FR-410 paper/live 账户

> **已迁移到 [v0.2-002-ui/spec.md UI-FR-410](../v0.2-002-ui/spec.md)**。

---

<a id="fr-420"></a>
### FR-420 实盘交易界面

> **已迁移到 [v0.2-002-ui/spec.md UI-FR-420](../v0.2-002-ui/spec.md)**。

---

<a id="fr-430"></a>
### FR-430 仿真交易界面

> **已迁移到 [v0.2-002-ui/spec.md UI-FR-430](../v0.2-002-ui/spec.md)**。

---

<a id="fr-440"></a>
### FR-440 dry-run 模式

| 有效需求 | 可测性 | 是否已决定 |
| -------- | ------ | ---------- |
| ✅        | ✅      | ✅          |

> **行为定义在本 FR;UI 表现已迁移到 [v0.2-002-ui/spec.md UI-FR-060](../v0.2-002-ui/spec.md)**。

允许正在实盘的策略进入 dry-run:策略正常运行但**不实际下单**。期间交易信号被记录,但策略指标无法计算。dry-run 期间的策略指标**参考其并行仿真实例**。


---

<a id="fr-450"></a>
### FR-450 消息通知（微信）— 事件定义

| 有效需求 | 可测性 | 是否已决定 |
| -------- | ------ | ---------- |
| ✅        | ✅      | ✅          |

> **通知配置 UI 已迁移到 [v0.2-002-ui/spec.md UI-FR-450](../v0.2-002-ui/spec.md)**。本 FR 仅定义**触发事件**。

以下事件发生时**应**触发微信通知(若用户已启用):

| #   | 事件             |
| --- | ---------------- |
| 1   | 委托提交         |
| 2   | 成交             |
| 3   | 委托失败         |
| 4   | 成交失败         |
| 5   | 实盘交易网关断开 |


---

<a id="fr-460"></a>
### FR-460 系统配置（init-wizard）

> **已迁移到 [v0.2-002-ui/spec.md UI-FR-460](../v0.2-002-ui/spec.md)**。

---

<a id="fr-470"></a>
### FR-470 安装与运行

| 有效需求 | 可测性 | 是否已决定 |
| -------- | ------ | ---------- |
| ✅        | ✅      | ✅          |

- 程序运行在 mac / linux / windows
- mac/linux：基于 shell 安装
- windows：基于图形界面安装
- 自带 embedded python + get-pip；创建虚拟运行环境
- 始终在虚拟运行环境下启动
- 以服务方式运行，并随开机自动启动

---

<a id="fr-480"></a>
### FR-480 数据重采样与移动平均工具

| 有效需求 | 可测性 | 是否已决定 |
| -------- | ------ | ---------- |
| ✅        | ✅      | ✅          |

框架提供面向历史行情分析与图表数据准备的 OHLCV 重采样工具。输入日线 DataFrame 后, 可按自然周或自然月聚合 open/high/low/close/volume/amount, 并在存在 `adjust` 字段时保留周期末复权因子。工具也可按给定周期列表计算收盘价移动平均列。

- 周线: 自然周聚合, open=周内第一交易日开盘, high=周内最高, low=周内最低, close=周内最后交易日收盘, volume/amount 求和
- 月线: 自然月聚合, 字段规则同周线
- 均线: 对 `close` 按窗口长度计算 rolling mean, 输出列名 `ma{period}`
- 空输入应返回空输出, 不伪造数据

---

<a id="fr-481"></a>
### FR-481 通知通道扩展（邮件 / 钉钉）

| 有效需求 | 可测性 | 是否已决定 |
| -------- | ------ | ---------- |
| ✅        | ✅      | ✅          |

除 FR-450 定义的通知事件枚举外, 框架提供邮件与钉钉机器人两类发送通道, 用于把已产生的交易/运行事件发送到用户配置的外部通知系统。通知通道只负责消息发送与格式封装, 不改变 FR-450 的事件枚举语义。

- 邮件通道支持纯文本、HTML 与附件 MIME 消息组装
- 邮件通道从运行配置读取发件人、收件人、SMTP 主机, 密码从环境变量读取
- 钉钉通道支持文本与 markdown 消息, 支持 access token 与 secret 加签
- 发送失败必须记录可观察错误, 不得吞掉失败并报告成功

---

<a id="fr-484"></a>
### FR-484 数据研究辅助工具

| 有效需求 | 可测性 | 是否已决定 |
| -------- | ------ | ---------- |
| ✅        | ✅      | ✅          |

框架提供用于研究/训练流程的数据辅助工具, 支持按时间顺序将时间序列数据切分为 train / valid / test 三个子集。该能力不参与策略运行撮合, 但作为数据研究工具需保持确定性与输入类型稳定。

- 支持 pandas DataFrame、Polars DataFrame 与 Polars LazyFrame 输入
- 支持按 `asset` 等分组列分别按时间排序切分; 也支持不分组整体切分
- 切分比例由 `(train_ratio, valid_ratio)` 控制, test 比例为剩余部分
- 输出类型应与输入类型一致, 且不改变原始业务列

---

<a id="fr-485"></a>
### FR-485 证券代码与市场规则工具

| 有效需求 | 可测性 | 是否已决定 |
| -------- | ------ | ---------- |
| ✅        | ✅      | ✅          |

框架提供证券代码格式转换与基础市场规则辅助函数, 用于在不同券商/数据源代码体系之间转换标的 ID, 并提供涨跌停、开盘时间等规则型工具输出。该能力承接 FR-270~300 的多数据源互通需求。

- 支持识别证券市场类型并在 hson / xt / jq 等代码格式之间转换
- 支持根据标的与行情数据计算或返回涨跌停辅助结果
- 支持给出开盘时间差等交易时段辅助值
- 转换失败或不支持的市场类型必须给出可判定错误或空结果, 不得返回误导性代码

