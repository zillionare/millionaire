...
> **范围**: 本节定义回测/paper/live/dry-run 启动与停止、风控绑定/解绑、运行状态查询等"模式切换与调度"控制 API. **测试工程师** 据此写 L1/L2 编排; **实现层** 据此暴露 HTTP endpoint. Request / Response schema 冻结到最小可用集, PR2 实施时若需扩展字段, **只增不删, 兼容旧调用方**.
>
> **本节不是 UI 契约** — 调度 UI 在 [v0.2-002-ui/spec.md UI-FR-040 / UI-FR-260](../v0.2-002-ui/spec.md). 本节只冻结"任何 UI 都要调用的 HTTP endpoint"层.

#### 3.3.1. 回测

##### `POST /broker/backtests`
...
```json
{
  ...
  "status": "pending",
  "run_id": "uuid",
  "created_at": "2022-12-30T10:00:00Z"
}
```

**错误码**: `STRATEGY_NOT_BACKTESTED` (409, 但本 endpoint 是"先跑回测", 不应触发) / `RISK_STRATEGY_NOT_BACKTESTABLE` (409) / `ALREADY_RUNNING` (409, 策略已有回测在跑) / 400 (参数错)

**关联 AC**: AC-230-01 / AC-230-02 / AC-230-03

##### `GET /broker/backtests/{backtest_id}`
...
```json
{
  ...
  "progress": 0.42,
  "started_at": "2022-12-30T10:00:00Z",
  "completed_at": null
}
```

**错误码**: `BACKTEST_NOT_FOUND` (404)

**关联 AC**: AC-230-04

##### `GET /broker/backtests/{backtest_id}/result`

**Response 200**: 返回 [§4.1 回测结果 JSON](./interfaces.md) (含 `nav_curve` / `trades` / 8 项指标)

**错误码**: `BACKTEST_NOT_FOUND` (404) / 409 (回测未完成)
...
> 注: 策略参数 `parameters` 不在 request 中, 来自回测 (FR-030 透传, [AC-230-02](./acceptance.md))。首次进 paper 必须先跑回测 (AC-230-01)。
...
**错误码**: `STRATEGY_NOT_BACKTESTED` (409, AC-230-01) / `ALREADY_RUNNING` (409) / 400 (参数错)

**关联 AC**: AC-230-02 / AC-230-03 / AC-240-02

##### `POST /broker/{strategy_id}/live`

**Request**: paper 基础 + `gateway_url`:

```json
{
  "initial_capital": 1000000.0,
  "slippage": 0.001,
  "commission": { "tax_rate": 0.001, "commission_rate": 0.00025, "min_commission": 5.0 },
  "gateway_url": "http://localhost:5001"
}
```

**Response 202**: 同 paper, `mode: "live"`

**错误码**: 同 paper

**关联 AC**: AC-230-02 / AC-230-03

##### `POST /broker/{strategy_id}/dry-run`

**Request**: `{}` (沿用 live 的 `initial_capital` / `slippage` / `commission`, **不**实际下单)
...
**错误码**: `ALREADY_RUNNING` (409) / 400 (参数错)

**关联 AC**: AC-440-01

##### `POST /broker/{strategy_id}/stop`
...
```json
{
  "portfolio_id": "uuid",
  "status": "stopped",
  "stopped_at": "2022-12-30T18:00:00Z"
}
```

**错误码**: 404 (策略当前无运行 portfolio) / 409 (策略已是 stopped)

**关联 AC**: AC-440-XX / FR-250

> **并行仿真 (FR-230 末尾)**: 当 live 启动时, 若原 paper 在跑, **当日**停止 paper 并自动创建并行仿真. 框架内部调度, 无需 UI 触发. 并行仿真的 portfolio 体现在 [§3.3.5 `GET /broker/{strategy_id}/portfolios`](#335-状态查询) 的 `parallel_to_live: true` 标记中.

#### 3.3.3. 风控策略绑定/解绑

##### `POST /broker/risk-strategies/{risk_strategy_id}/bind`

**Request**:

```json
{
  "host_strategy_id": "user.MyStrategy",
  "parameters": { "m": 7.0, "k": 0.5, "n": 1 }
}
```
...
{
  "binding_id": "uuid",
  "risk_strategy_id": "user.DrawbackSell",
  "host_strategy_id": "user.MyStrategy",
  "status": "active",
  "activated_at": "2022-12-30T10:00:00Z"
}
```

**错误码**: 404 (risk_strategy_id 不存在) / 400 (参数错)

**关联 AC**: AC-250-01 / AC-130-02

##### `POST /broker/risk-strategies/{risk_strategy_id}/unbind`
...
```json
{
  "binding_id": "uuid",
  "status": "unbound",
  "unbound_at": "2022-12-30T18:00:00Z"
}
```

**错误码**: 404 (binding_id 不存在) / 409 (已 unbound)

**关联 AC**: FR-013 生命周期

> **自动激活 (AC-250-01)**: 宿主进 paper/live 时, 关联风控**自动**激活, 不需要显式 bind. bind endpoint 是"显式预绑定"或"在宿主进 paper 前配置风控参数"用. 显式 bind 后的 binding 在宿主进 paper 时**自动**复用, 无需重复调用.

#### 3.3.4. 错误码 (跨 §3.3.1 ~ §3.3.3)

| HTTP | code                          | 触发                                                                       | 关联 AC         |
| ---- | ----------------------------- | -------------------------------------------------------------------------- | --------------- |
| 409  | `STRATEGY_NOT_BACKTESTED`     | 独立策略未跑过回测, 直接尝试进 paper                                       | AC-230-01       |
| 409  | `RISK_STRATEGY_NOT_BACKTESTABLE` | BacktestRunner 检测到 RiskStrategy 实例                                    | AC-130-01       |
| 409  | `ALREADY_RUNNING`             | 策略已在 paper/live/dry-run 某模式运行, 启动被拒                            | AC-230-XX       |
| 404  | `BACKTEST_NOT_FOUND`          | `GET /broker/backtests/{backtest_id}` 找不到                                | AC-230-04       |
| 400  | `INVALID_PARAMETERS`          | Request body 字段缺失 / 类型错 / 越界                                       | 通用            |
| 404  | `STRATEGY_NOT_FOUND`          | `strategy_id` / `risk_strategy_id` / `host_strategy_id` 不存在               | 通用            |
| 500  | `BROKER_RUNTIME_ERROR`        | 框架内部错误 (撮合失败 / DB 写失败 / 调度失败); 详见 [§5 异常错误码表](./interfaces.md) | 通用        |

#### 3.3.5. 运行状态查询

##### `GET /broker/{strategy_id}/portfolios`
  ...
  "portfolios": [
    {
      ...
      "status": "running",
      "started_at": "2022-12-30T10:00:00Z",
      "parallel_to_live": false
    },
    {
      "portfolio_id": "uuid",
      "mode": "paper",
      "status": "running",
      "started_at": "2022-12-30T10:00:01Z",
      "parallel_to_live": true
    }
  ]
}
```

> **并行仿真查询**: `parallel_to_live: true` 标记表示"由 live 启动时自动创建的并行仿真" (FR-230 末尾)。`status` 为 `running` 表示在跑, `stopped` 表示已停止。

**错误码**: `STRATEGY_NOT_FOUND` (404)

**关联 AC**: FR-230 / FR-250 / AC-440-XX

##### `GET /broker/portfolios/{portfolio_id}/status`
...
```json
{
  ...
  "status": "running",
  "started_at": "2022-12-30T10:00:00Z",
  "last_tick_at": "2022-12-30T14:30:00Z",
  "orders_count": 42,
  "fills_count": 38,
  "assets": {
    "cash": 950000.0,
...
> `assets` 字段对应 [interfaces.md §4.5](./interfaces.md) 表 schema。`last_tick_at` 用于 L1/L2 paper E2E 监控推进状态。

**错误码**: `PORTFOLIO_NOT_FOUND` (404)

**关联 AC**: FR-230 / FR-250 / AC-440-XX

#### 3.3.6. §3.3 完整覆盖矩阵 (回应 §6.6 断言依据清单)

| 行为 | 接口 / 表 | 关联 AC |
| ---- | --------- | ------- |
| 启动回测 / 路径 1 / 路径 2 | [§3.3.1](./interfaces.md) `POST /broker/backtests` | AC-230-01/02/03 |
| 回测状态查询 / 30 上限 | [§3.3.1](./interfaces.md) `GET /broker/backtests/{id}` | AC-230-04 |
| 回测结果 (净值 / 指标 / 8 项) | [§3.3.1](./interfaces.md) `GET .../result` + [§4.1](./interfaces.md) | AC-340/350 |
| 启动 paper / live / dry-run | [§3.3.2](./interfaces.md) `POST /broker/{id}/{paper|live|dry-run}` | AC-230-02/03 / AC-240-02 / AC-440-01 |
| 停止 paper / live / dry-run | [§3.3.2](./interfaces.md) `POST /broker/{id}/stop` | AC-440-XX / FR-250 |
| 风控绑定 / 自动激活 | [§3.3.3](./interfaces.md) `POST .../bind` | AC-250-01 / AC-130-02 |
| 风控解绑 | [§3.3.3](./interfaces.md) `POST .../unbind` | FR-013 生命周期 |
| 运行状态查询 / 并行仿真 | [§3.3.5](./interfaces.md) `GET /broker/{id}/portfolios` + `GET /broker/portfolios/{id}/status` | FR-230 / FR-250 / AC-440-XX |
