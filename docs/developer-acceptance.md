...
```bash
conda activate quantide
```

如果当前 shell 没有激活环境，则在命令前加：

```bash
conda run -n quantide <command>
```
...
```bash
conda run -n quantide poetry run pytest -m "e2e and release_gate" tests/e2e -q
```
...
```bash
conda run -n quantide pytest tests/e2e/three_mode/test_dual_ma_parity.py -q
```
...
```bash
conda run -n quantide pytest tests/e2e/web/test_dev_stub_mode.py -q
```
