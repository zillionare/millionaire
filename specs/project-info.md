...
- **Test PR**: 已跳过（`|| true` 容错路径；不在 v0.2 阶段创建对 main 的临时 PR）
- **Story Source**: `specs/001-strategy-framework-v0.2/story.md`（351 行原文）
- **Spec 编号决策**: `specs/` 目录为空 → 自动分配 `001`；keyword 从 story §1「策略框架」提取为 `strategy-framework`
- **Repo 状态**: 已存在
...
## 备注
- **故事单源**：`specs/001-strategy-framework-v0.2/story.md`（351 行）；`.dev/specs/story.md` 是指向它的软链，供习惯从 `.dev/specs/` 读 story 的下游 Agent 沿用旧路径。
