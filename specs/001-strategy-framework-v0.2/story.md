...
> **不在本文件范围**：用户可见的 Web 界面（账户视图、图表、委托/成交查询、交易界面、init-wizard 等），已迁移到 [002-ui-v0.2/story.md](../002-ui-v0.2/story.md)。本文件中若子句涉及 UI 呈现，会以"（UI 部分见 002-ui-v0.2/story.md §X）"标注。
...
与策略参数不同，运行时参数在每次切换运行模式（回测 → 仿真 → 实盘）时都允许重设（见 [002-ui-v0.2/story.md §2 运行时参数编辑](../002-ui-v0.2/story.md#2-运行时参数编辑)），不被回测结果锁定。
...
### 3.5. 数据同步
 ...
 - 盘前获取当日涨跌停价格、复权因子获取
 - 定时任务可配置，运行有报告跟踪，错误状态可被报告（UI 报告通道见 [002-ui-v0.2/story.md §13 数据同步任务的错误展示](../002-ui-v0.2/story.md#13-数据同步任务的错误展示)）
 - 任务错过执行（错误纠正后）重新执行
 - 数据补录

### 3.6. 数据完整性校验及报告
后台任务，定时执行数据完整性校验；错误状态可被报告（UI 报告通道见 [002-ui-v0.2/story.md §14 数据完整性校验报告展示](../002-ui-v0.2/story.md#14-数据完整性校验报告展示)）。

### 3.7. 数据查询支持
- 支持用户查询交易日历
- 支持用户按名字、数字和拼音模糊查询个股基本信息
- 支持用户查询个股历史行情（K 线图渲染见 [002-ui-v0.2/story.md §15 个股查询与 K 线图](../002-ui-v0.2/story.md#15-个股查询与-k-线图)）
...
### 4.4. 可视化
图表选型与渲染规则见 [002-ui-v0.2/story.md §6 核心图表](../002-ui-v0.2/story.md#6-核心图表)。本节仅说明图表所消费的数据来源：
...
### 4.5. 回测进度可视化
回测过程中的实时绘制与进度展示见 [002-ui-v0.2/story.md §5 回测进度可视化](../002-ui-v0.2/story.md#5-回测进度可视化)。

## 5. 账户管理

> 本节仅说明**业务侧账户模型**：每个日线策略和日内策略拥有独立资金账户（虚拟账户）；风控策略无独立账户，其交易归属宿主策略账户。**UI 呈现与查询交互**（账户总览、委托/成交查询、paper/live 净值曲线与盈亏排行等）已迁移到 002-ui-v0.2。

- 账户总览与单策略详情：见 [002-ui-v0.2/story.md §8 账户总览](../002-ui-v0.2/story.md#8-账户总览)
- 委托 / 成交记录：见 [002-ui-v0.2/story.md §9 委托和成交记录](../002-ui-v0.2/story.md#9-委托和成交记录)
- paper / live 账户查询：见 [002-ui-v0.2/story.md §10 paper--live-账户](../002-ui-v0.2/story.md#10-paper--live-账户)

## 6. 实盘交易界面

> 业务规则：界面顶部提供**策略选择器**，列出当前所有实盘运行的策略（日线策略 + 日内策略），用户可切换。手动交易必须归属一个目标策略（虚拟账户）；风控策略不可作为手动交易目标。**完整 UI 契约**见 [002-ui-v0.2/story.md §11 实盘交易界面](../002-ui-v0.2/story.md#11-实盘交易界面)。

## 7. 仿真交易界面

> 业务规则：界面顶部提供**策略选择器**，列出当前所有仿真运行的策略，用户可切换；提供补单入口（防止自动化失败的补救措施）。**完整 UI 契约**见 [002-ui-v0.2/story.md §12 仿真交易界面](../002-ui-v0.2/story.md#12-仿真交易界面)。
...
> 通知渠道的**配置 UI**（二维码扫描启用、事件订阅勾选、静默时段等）见 [002-ui-v0.2/story.md §16 通知配置界面](../002-ui-v0.2/story.md#16-通知配置界面)。

## 10. 系统配置

> 业务规则：系统安装后首次启动必须运行 init-wizard（通过 web）以引导用户配置关键项（登录密码、qmt-gateway 连接、通知渠道、数据源、数据目录）。**完整 UI 契约与流程步骤**见 [002-ui-v0.2/story.md §17 系统配置（init-wizard）](../002-ui-v0.2/story.md#17-系统配置init-wizard)。
